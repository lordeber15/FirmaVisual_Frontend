            ...
            >
                ...
                {docs.slice(0, 3).map(doc => (
                  <div key={doc.id} className="flex items-center justify-between py-1.5 text-xs">
                    ...
                    <span className={`ml-2 px-2 py-0.5 rounded-full font-bold ${
                      ...
                      : doc.status === 'PARTIAL' ? 'bg-primary-50 text-primary'
              ...
              <div className="border-t border-slate-100 px-6 py-3 flex items-center justify-between">
                <Link
                  ...
                  className="flex items-center text-sm font-bold text-primary hover:text-primary-700 transition-colors"
                >
                ...
                {canManage && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-bold text-secondary hover:text-secondary-600 transition-colors"
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white text-slate-900 border border-slate-200 rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-primary resize-none font-medium"
                  />
                </div>
              </div>
                    ...
                    {availableUsers.map((u) => (
                      <label
                        ...
                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${
                          selectedMemberIds.includes(u.id) ? 'border-secondary-300 bg-secondary-50' : 'border-slate-100 hover:bg-slate-50'
                        }`}
                      >
                        <div className="flex items-center space-x-3">
                          <input type="checkbox" checked={selectedMemberIds.includes(u.id)} onChange={() => toggleMember(u.id)} className="w-4 h-4 rounded text-secondary" />
                          <div>
